...
for i, name_method in enumerate(METHODS):
    ...
    ris = np.load(DIR_EXPERIMENT_method + f'/mean_time_{name_method}.npy', allow_pickle=True).tolist()
    mean_time = {**mean_time, **ris}

print(mean_time)

Dimensions = {'BP_particle_coop':[64, 128, 256, 512, 1024, 2048, 4096, 8192],
              'LSTM_MPNN':[2, 4, 8, 16, 32, 64, 128, 256, 512, 1024, 2048]}

ax_handler = []
for i, name_method in enumerate(METHODS):

    y_1 = np.array([v for k,v in mean_time.items() if name_method in k]).squeeze()
    y_mean = np.mean(y_1,1)
    y_up = np.max(y_1,1) - y_mean
    y_down = y_mean - np.min(y_1,1)
    yerr = [y_down, # 'down' error
            y_up]  # 'up' error

    ax_handler.append(ax_error[0].errorbar(Dimensions[name_method], y_mean, yerr=yerr, capsize=3, fmt="r--o", linestyle=line_style_indexes_per_method[name_method], ecolor = "black", color=colors[color_indexes_per_method[name_method]]))
    plt.grid()

    for _ in Dimensions[name_method]:
        ax_error[0].axvline(_, color='black', linestyle = '--', linewidth = 1, alpha = 0.5)



ax_error[0].legend(ax_handler, [k for k in METHODS], loc='best', shadow=False)
ax_error[0].set(xlabel='Noise')
ax_error[0].set(ylabel='Time [s]')
