...
for mc in tqdm(range(MC_iterations)):
    ...
    for n in range(dataset_params['number_instants']):
        ...
        # dataset_instance.compute_next_step()
        ...
        #                 print('Prior mean x: n {} a {} {}'.format(n, agent, mean.squeeze().tolist()))
                        ...
                        # plot prior
                        # plot_particles(BP_instance.beliefs[agent][0][0,:].reshape([-1,]), BP_instance.beliefs[agent][0][1,:].reshape([-1,]), BP_instance.beliefs[agent][1].reshape([-1,]))

        # BP
        # TEST PERFORMANCES
        # cProfile.run('BP_instance.prediction(n)')
        # cProfile.run('BP_instance.update(dataset_instance, n)')
        # cProfile.run('BP_instance.estimate_position(n)')
...
np.save(DIR_EXPERIMENT + f'/rmse_x_predicted_{name_method}.npy', [RMSE_per_step, x_per_step], allow_pickle = True) 
    ...
    # NO PREDICTIONS
    # ax_scenario, ax_error = plot_step_artificial_dataset(dataset_params, model_params, dataset_instance, ax_scenario, ax_error, time_n=n, DIR_EXPERIMENT = DIR_EXPERIMENT)
    # time.sleep(0.01)
