...
while mc < MC_iterations:
    ...
    if np.mean(RMSE_per_step[f'pos_{name_method}']) < 1.5:
        mc +=1
...
for agent in range(dataset_params['num_agents']):
    ax_scenario.plot(pos_GT[agent,:,0], pos_GT[agent,:,1], '-', color=colors[-1])

# plot method
pos_method = np.array(x_per_step[f'pos_{name_method}'])
for agent in range(dataset_params['num_agents']):
    ax_scenario.plot(pos_method[agent,:,0], pos_method[agent,:,1], '--', color=colors[0])

RMSE_pos_per_step_method = RMSE_per_step[f'pos_{name_method}'] 
ax_error[0].plot(range(len(RMSE_pos_per_step_method)), RMSE_pos_per_step_method, color=colors[0])
...
if len(ax_error) >= 2:
    ...
    ax_error[1].plot(range(len(RMSE_vel_per_step_method)), RMSE_vel_per_step_method, color=colors[0])
    ...
    if len(ax_error) >= 3:
        ...
        ax_error[2].plot(range(len(RMSE_acc_per_step_method)), RMSE_acc_per_step_method, color=colors[0])
...
ax_error[0].legend(name_method, loc='best', shadow=False)
ax_error[0].set(xlabel='Time n')
ax_error[0].set(ylabel='RMSE pos [m]')
if len(ax_error) >= 2:
    ax_error[1].legend(name_method, loc='best', shadow=False)
    ax_error[1].set(xlabel='Time n')
    ax_error[1].set(ylabel='RMSE vel [m/s]')
    if len(ax_error) >= 3:
        ax_error[2].legend(name_method, loc='best', shadow=False)
