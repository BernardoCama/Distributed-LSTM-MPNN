...
mean_RMSE_pos = []
for agent_idx, noise in enumerate(AGENTS):
    mean_RMSE_pos.append([])
    for i, name_method in enumerate(METHODS):
        mean_RMSE_pos[agent_idx].append(mean_RMSE[f'pos_{name_method}_{noise}'])
...
if len(ax_error) >= 2:
    ...
    for agent_idx, noise in enumerate(AGENTS):
        ...
        for i, name_method in enumerate(METHODS):
            mean_RMSE_vel[agent_idx].append(mean_RMSE[f'vel_{name_method}_{noise}'])
    ...
    if len(ax_error) >= 3:
        ...
        for agent_idx, noise in enumerate(AGENTS):
            ...
            for i, name_method in enumerate(METHODS):
                mean_RMSE_acc[agent_idx].append(mean_RMSE[f'acc_{name_method}_{noise}'])
        mean_RMSE_acc = np.array(mean_RMSE_acc)

for i, name_method in enumerate(METHODS):
    ax_error[0].plot(AGENTS, mean_RMSE_pos[:,i], linestyle=line_style_indexes_per_method[name_method], color=colors[color_indexes_per_method[name_method]])
    ax_error[0].legend([k for k in METHODS], loc='best', shadow=False)
    ax_error[0].set(xlabel='Agents')
    ax_error[0].set(ylabel='RMSE pos [m]')
    ax_error[0].set_xticks(AGENTS)
    ax_error[0].set_xticklabels([str(int(val)) for val in AGENTS])
