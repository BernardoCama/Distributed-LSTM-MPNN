...
training_validation_dataset_params = {
                ...
                'instances':1000,                          # 1000, useful to increase, 10000 server
}
dataset_params = {'load_dataset':0,
                  ...
                  'std_noise_measurements':10, # inter-distances [m]
                  'std_noise_gnss_position':10,
                  'std_noise_gnss_velocity':3,
                  'std_noise_gnss_acceleration':0,
}
model_params = {'centralized': 0,                                           # 0 for Distributed training
                'distributed_strategy': 4,                                  # 1, 2, 3, 4 (consensus)
    
                'log_MPNN_LSTM':0,
                'log_BP':0,
                'batch_size_length_seq':dataset_params['number_instants']-1,   # batch size is the length of the sequence for the training of lstm
                'batch_size_real':32,                                  # number of instances to be trained simultaneously 
                
                'NUM_MEAS': 1, 
                'NUM_MEAS_GNSS':4,
                'T_message_steps':15,
                'num_regress_steps':15-1,
